// Game: Learn the Letter Ta (ت) - Arabic
// Category: Arabic Letters (أبتثج)
// Type: Standard selection - introduction to the letter ت
// Instructions in Hebrew, word audio in Arabic (spoken/colloquial)
// Words: تفاح (tuffah/apple), توت (toot/berries), تاج (taj/crown)

const learnTaAr = {
  id: 'learn-ta-ar',
  categoryId: 'arabic-alef-jim',
  name: 'ت',
  description: 'למדו את האות ت',
  icon: 'ت',
  type: 'selection',
  gameType: 'standard',
  availableIn: ['easy'],

  questions: {
    easy: [
      // Question 1: Introduction - show letter + example words
      {
        id: 'lt-ar-e1',
        instruction: 'זוהי האות ت. המילים הבאות מתחילות באות ت',
        instructionAudio: '/audio/ar/intro-ta.mp3',
        questionImage: 'letter-ta-ar',
        isIntroMultiSelect: true,
        options: [
          { id: 'opt-1', image: 'word-tapuach', label: 'تفاح', isCorrect: true, optionAudio: '/audio/ar/word-tuffah.mp3' },
          { id: 'opt-2', image: 'word-tut', label: 'توت', isCorrect: true, optionAudio: '/audio/ar/word-toot.mp3' },
          { id: 'opt-3', image: 'word-taj', label: 'تاج', isCorrect: true, optionAudio: '/audio/ar/word-taj.mp3' },
        ],
      },
      // Question 2: Select all ت letters from 3x3 grid
      {
        id: 'lt-ar-e2',
        instruction: 'בחרו את כל האותיות ت',
        instructionAudio: '/audio/ar/select-all-ta.mp3',
        multiSelect: true,
        options: [
          { id: 'opt-1', image: 'letter-ta-ar', label: 'ت', isCorrect: true },
          { id: 'opt-2', image: 'letter-ba-ar', label: 'ب' },
          { id: 'opt-3', image: 'letter-alef-ar', label: 'أ' },
          { id: 'opt-4', image: 'letter-tha-ar', label: 'ث' },
          { id: 'opt-5', image: 'letter-ta-ar', label: 'ت', isCorrect: true },
          { id: 'opt-6', image: 'letter-jim-ar', label: 'ج' },
          { id: 'opt-7', image: 'letter-ta-ar', label: 'ت', isCorrect: true },
          { id: 'opt-8', image: 'letter-ba-ar', label: 'ب' },
          { id: 'opt-9', image: 'letter-alef-ar', label: 'أ' },
        ],
        correctCount: 3,
      },
      // Question 3: Find letter ت among 6 letters
      {
        id: 'lt-ar-e3',
        instruction: 'מצאו את האות ت',
        instructionAudio: '/audio/ar/find-ta.mp3',
        options: [
          { id: 'opt-1', image: 'letter-ba-ar', label: 'ب' },
          { id: 'opt-2', image: 'letter-alef-ar', label: 'أ' },
          { id: 'opt-3', image: 'letter-jim-ar', label: 'ج' },
          { id: 'opt-4', image: 'letter-ta-ar', label: 'ت', isCorrect: true },
          { id: 'opt-5', image: 'letter-tha-ar', label: 'ث' },
          { id: 'opt-6', image: 'letter-ba-ar', label: 'ب' },
        ],
      },
      // Question 4: Which word starts with ت
      {
        id: 'lt-ar-e4',
        instruction: 'איזו מילה מתחילה באות ت?',
        instructionAudio: '/audio/ar/word-starts-ta.mp3',
        options: [
          { id: 'opt-1', image: 'word-tapuach', label: 'تفاح', isCorrect: true, optionAudio: '/audio/ar/word-tuffah.mp3' },
          { id: 'opt-2', image: 'word-rabbit', label: 'أرنب' },
          { id: 'opt-3', image: 'word-barvaz', label: 'بطة' },
          { id: 'opt-4', image: 'word-gamal', label: 'جمل' },
        ],
      },
      // Question 5: Review previous letter - Find ب
      {
        id: 'lt-ar-e5',
        instruction: 'מצאו את האות ب',
        instructionAudio: '/audio/ar/find-ba.mp3',
        options: [
          { id: 'opt-1', image: 'letter-ta-ar', label: 'ت' },
          { id: 'opt-2', image: 'letter-alef-ar', label: 'أ' },
          { id: 'opt-3', image: 'letter-ba-ar', label: 'ب', isCorrect: true },
          { id: 'opt-4', image: 'letter-jim-ar', label: 'ج' },
          { id: 'opt-5', image: 'letter-tha-ar', label: 'ث' },
          { id: 'opt-6', image: 'letter-ta-ar', label: 'ت' },
        ],
      },
      // Question 6: MULTI-SELECT - Mark ALL أ, ب, ت letters
      {
        id: 'lt-ar-e6',
        instruction: 'סמנו את כל האותיות أ, ب ו-ت',
        instructionAudio: '/audio/ar/mark-all-alef-ta.mp3',
        multiSelect: true,
        options: [
          { id: 'opt-1', image: 'letter-alef-ar', label: 'أ', isCorrect: true },
          { id: 'opt-2', image: 'letter-jim-ar', label: 'ج' },
          { id: 'opt-3', image: 'letter-ba-ar', label: 'ب', isCorrect: true },
          { id: 'opt-4', image: 'letter-tha-ar', label: 'ث' },
          { id: 'opt-5', image: 'letter-ta-ar', label: 'ت', isCorrect: true },
          { id: 'opt-6', image: 'letter-jim-ar', label: 'ج' },
          { id: 'opt-7', image: 'letter-alef-ar', label: 'أ', isCorrect: true },
          { id: 'opt-8', image: 'letter-tha-ar', label: 'ث' },
          { id: 'opt-9', image: 'letter-jim-ar', label: 'ج' },
          { id: 'opt-10', image: 'letter-tha-ar', label: 'ث' },
          { id: 'opt-11', image: 'letter-jim-ar', label: 'ج' },
          { id: 'opt-12', image: 'letter-tha-ar', label: 'ث' },
        ],
        correctCount: 4,
      },
    ],
  },
};

export default learnTaAr;
